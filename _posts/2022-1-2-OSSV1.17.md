---
layout: post
title: DGHack 2021 in-depth analysis - Instakilogram
---
...
Local flag n°1: `DGA{local_flag_01}` (unfortunately the server is offline so I cannot get the real flags anymore 😞)
...
This translates to `\xcd\x10\xc3` in hexadecimal. Sending this crafted input to the program will call the 16th handler which will print the first part of the flag.

The second part is also behind a handler code check (`0x16`) but there are also other values on the stack that are checked. Since figuring out the expected stack layout is a bit complicated with static analysis I choose to craft an absolute jump payload to `0x1585` which is inside the subroutine that prints the second part.

```x86asm
mov dword ptr [rsp], 0x1585
mov dword ptr [rsp+4], 0x0
ret
```

This translates to `\xc7\x04\x24\x85\x15\x00\x00\xc7\x44\x24\x04\x00\x00\x00\x00\xc3` in hexadecimal. Sending this crafted input to the program will jump inside `sub_156D` which will print the second part of the flag.
...
The documentation to jump back to real mode can be found at [https://wiki.osdev.org/ATA_PIO_Mode](https://wiki.osdev.org/ATA_PIO_Mode).

Choose the one you prefer and voilà!
...
On the other hand, I learned about the inner workings of boot sequences, real and protected mode as well as port I/O.